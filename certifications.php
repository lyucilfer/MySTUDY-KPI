...
if(isset($_POST['submit_certification'])){
   ...
   $certification_dates = mysqli_real_escape_string($conn, $_POST['certification_dates']);
   ...
   if(mysqli_num_rows($check_user_form_query) > 0) {
       ...
       if(!preg_match($datePattern, $certification_dates)) {
       ...
       elseif(empty($message)) {
             $insert_certifications = mysqli_query($conn, "INSERT INTO certifications_form(id, sem, year, certifications, certification_dates, remarks, docs) VALUES('$user_id', '$sem', '$year', '$certifications', '$certification_dates', '$remarks', '$docs')") or die('query failed');
             if($insert_certifications) {
...
<h1 class="heading">Certification Management</h1>
                      ...
                      <div class="container">
                        <div class="table-wrapper">
                          <table class="table">
                              ...
                              <tbody>
                                  <?php
                                        $sql = "SELECT * FROM certifications_form";
                                        ...
                                        if (mysqli_num_rows($result) > 0) {
                                            ...
                                            while($row = mysqli_fetch_assoc($result)) {
                                                ...
                                                echo "<td data-th='NAME OF CERTIFICATIONS'>" . $row["certifications"] . "</td>";
                                                echo "<td data-th='CERTIFICATION DATES'>" . $row["certification_dates"] . "</td>";
                                                echo "<td data-th='REMARKS'>" . $row["remarks"] . "</td>";
                                                echo "<td data-th='DOCUMENTS'>" . $row["docs"] . "</td>";
                                                echo '<td data-th="ACTION"><a class="btn" href="updatelists.php?source=certifications&id=' . $row["cer_id"] . '">Edit</a>' . " " . '<a class="btn" href="delete.php?source=certifications&id=' . $row["cer_id"] . '" onClick="return confirm(\'Delete?\');">Delete</a> </td>';
                                                echo "</tr>" . "\n\t\t";
                                                $numrow++;
                                            }
                                        } else {
...
<section class="table-container">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <input type="text" name="certifications" required placeholder="Enter your certification name" class="box">
      <p>Certification Date</p>
      <input type="text" name="certification_dates" required placeholder="Enter the certification date (DD/MM/YYYY)" class="box">
      <p>Remarks</p>
      <input type="text" name="remarks" placeholder="Enter your remarks" class="box">
      <p>Add Your Certification Documents</p>
      <input type="file" name="docs" accept="image/*" class="box">
      <input type="submit" value="Submit" name="submit_certification" class="btn">
   </form>
